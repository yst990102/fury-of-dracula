...
int main(void)
{
	...
	{///////////////////////////////////////////////////////////////////
 	...
 	{///////////////////////////////////////////////////////////////////
		...
		printf("Testing trap locations after one fails of trail\n");
		
		char *trail =
			"GBD.... SGE.... HGE.... MGE.... DKLT... " // klausenbury
			"GSZ.... SGE.... HGE.... MGE.... DGAT... " // galatz
			"GBE.... SGE.... HGE.... MGE.... DCNT... " // constanta
			"GBE.... SGE.... HGE.... MGE.... DVR.... " // verna
			"GBE.... SGE.... HGE.... MGE.... DSO.... " // sofia
			"GBE.... SGE.... HGE.... MGE.... DSA.... " // salonica
			"GBE.... SGE.... HGE.... MGE.... DVA..M. "; // valona
		...
		int numTraps = 0;
		PlaceId *traps = GvGetTrapLocations(gv, &numTraps);
		for (int i = 0; i < numTraps; i++) printf("    traps  @ %-15s\n", placeIdToName(traps[i]));
		assert(numTraps == 2);
		sortPlaces(traps, numTraps);
		assert(traps[1] == GALATZ && traps[0] == CONSTANTA);
		free(traps);
	...
	{///////////////////////////////////////////////////////////////////
...

		{
			...
			PlaceId *locs = GvGetReachableByType(gv, PLAYER_LORD_GODALMING,
			                                     2, PARIS, false, true,
			                                     false, &numLocs);
			for (int i = 0; i < numLocs; i++) printf("   rail  @ %-15s\n", placeIdToName(locs[i]));
			assert(numLocs == 7);
			sortPlaces(locs, numLocs);
		...
		
		{
			...
			PlaceId *locs = GvGetReachableByType(gv, PLAYER_LORD_GODALMING,
			                                     ...
			                                     false, &numLocs);
			printf("%d       \n", numLocs);
		    for (int i = 0; i < numLocs; i++) printf("   locs  @ %-15s\n", placeIdToName(locs[i]));
